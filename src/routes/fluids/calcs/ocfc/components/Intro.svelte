...
{#if isVisible}
	<section in:fly={{ y: -200 }} out:fly={{ y: -200, delay: 100 }}>
		This calculator on this page is for <span style="font-weight: bold; letter-spacing: -0.03em">normal</span> (uniform)
		flow and
		...
		<p />
		Clicking on a result panel will display the associated calculations for each result. Clicking again will close the panel.
		<p />
		Either flow depth, {@html ki(`y`)}, in metres ({@html ki(`\\mathsf{m}`)}) or volume flow,
		{@html ki(`Q`)}, in cubic metres per second ({@html ki(`\\mathsf{m^3\\!/s}`)}) must be specified. But not both.
		<p />
		...
		{#if typeOfChannel !== "triangular"}
			...
			channel, {@html ki(`y`)} cannot be determined analytically from {@html ki(`Q`)} and is found using an iterative method.
		{/if}
		<p />
		<br />
		By default, this calculator uses {@html ki(`3`)} significant digits for input values and answers (or {@html ki(`4`)}
		significant digits if the first non-zero digit is a {@html ki(`1`)}, e.g.{@html ki(`0.234`)} but {@html ki(
			`0.1234`
		)}). Interim calculations are to {@html ki(`6`)} significant digits (or {@html ki(`7`)} in the case of a leading {@html ki(
			`1`
		)}) to avoid the accumulation of rounding errors. You can alter these default values here:
		<p />
		{extraDig}
		<div class="digs">
			<div class="sdigs">
