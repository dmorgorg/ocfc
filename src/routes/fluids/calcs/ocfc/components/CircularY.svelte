	...
	<section class="fig">
		...
		<form>
			<div class="lower-inputs">
				<label class="slope">
					{@html ki(`\\large S=`)}
					<input type="number" step="any" id="slope" bind:value={ss} on:input={processChange} />
					{@html ki(`\\small\\%`)}
				</label>
				<label class="n">
					Mannings {@html ki(`\\large n =`)}
					<input type="number" step="any" id="n" bind:value={ns} on:input={processChange} />
					{@html ki(`\\mathsf{m}`)}
				</label>
				<label class="g">
					{@html ki(`\\large g=`)}
					<input type="number" step="any" id="g" bind:value={gs} on:input={processChange} />
		...
		`)} />
			{:else if y > D / 2}
				...
				\\begin{aligned}
					...
					&= \\cos^{-1} \\left(\\frac{${sds(y)}\\, \\mathsf{m}-${r}\\, \\mathsf{m}}{${r}\\, \\mathsf{m}} \\right)\\\\\\\\
					\\alpha &= ${sdw(alpha)}^\\circ
				\\end{aligned}
			`)} />
				<Card
